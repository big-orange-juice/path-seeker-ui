...
import { Archive, Compass, Map, Play, Sparkles } from "lucide-vue-next"
...
const EXPANDED_ITEM_WIDTH = 86
const ITEM_HEIGHT = 36
const ITEM_GAP = 7
const CONTAINER_PADDING = 5
const SELECTION_SETTLE_DURATION = 0.18
const COLLAPSED_WIDTH = EXPANDED_ITEM_WIDTH + CONTAINER_PADDING * 2
...
const shellItems: Array<{ label: string; value: ShellTab; to: string; icon: typeof Compass }> = [
  { label: "大厅", value: "hall", to: "/shell/hall", icon: Compass },
  { label: "任务", value: "playing", to: "/shell/playing", icon: Play },
  { label: "归档", value: "archive", to: "/shell/archive", icon: Archive },
]
...
const activeChapterMapPath = computed(() =>
  missionStore.activeSession ? `/missions/${missionStore.activeSession.routeId}/map` : "/shell/playing",
)

const actions = computed<FabActionItem[]>(() => {
  if (onShellRoute.value) {
    return shellItems.map((item) => ({
      key: item.value,
      label: item.label,
      icon: item.icon,
      to: item.to,
      active: shellTab.value === item.value,
    }))
  }
    ...
    return [
      {
        ...
        label: "章节",
        icon: Map,
        to: routeId.value ? `/missions/${routeId.value}/map` : activeChapterMapPath.value,
        active: onMapPage,
      },
      {
        key: "resume",
        ...
        active: !onMapPage && onPuzzleFlowPage,
      },
      {
        key: "hall",
        label: "大厅",
...
const activeItem = computed(() => actions.value[activeIndex.value] || actions.value[0] || null)
...
const islandStyle = computed<CSSProperties>(() => {
  const width = COLLAPSED_WIDTH + (expandedWidth.value - COLLAPSED_WIDTH) * expandedProgress.value
...
const indicatorStyle = computed<CSSProperties>(() => {
  const x = CONTAINER_PADDING + activeIndex.value * (EXPANDED_ITEM_WIDTH + ITEM_GAP)
      ...
      <div class="fab-shell">
        ...
        >
          ...
          >
            <span class="fab-collapsed-item">
              <span class="fab-icon fab-icon-primary">
                <component :is="activeItem.icon" class="h-[0.95rem] w-[0.95rem]" />
              </span>
          ...
          <div v-if="showRail" class="fab-rail">
            ...
            >
              <span class="fab-icon" :class="{ 'fab-icon-primary': displayedKey === item.key }">
...
.fab-collapsed-item {
  ...
  gap: 0.44rem;
  border-radius: 999px;
  background: linear-gradient(135deg, rgba(88, 70, 36, 0.94), rgba(209, 178, 111, 0.2));
  box-shadow:
