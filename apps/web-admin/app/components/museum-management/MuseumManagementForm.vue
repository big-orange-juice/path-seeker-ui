  ...
  <form class="space-y-4" @submit.prevent="emit('save')">
      <section class="grid gap-3 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <UiInput v-model="model.museumCode" placeholder="如 SHM-EAST" />
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">主体名称</label>
          <UiInput v-model="model.name" placeholder="请输入主体名称" />
        </div>
      </section>

      <section class="space-y-2">
        <label class="text-sm font-medium text-foreground">主体地址</label>
        <UiInput v-model="model.address" placeholder="请输入主体地址" />
      </section>

      <section class="grid gap-3 md:grid-cols-2">
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">开放时间</label>
          <UiInput v-model="model.openingHours" placeholder="如 10:00 - 18:00" />
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">闭馆日</label>
          <UiInput v-model="model.closedDay" placeholder="如 每周一" />
        </div>
      </section>
      ...
      <section class="grid gap-3 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <UiInput v-model="model.contactPhone" placeholder="请输入联系电话" />
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">微信公众号</label>
          <UiInput v-model="model.wechatAccount" placeholder="请输入公众号名称" />
        </div>
      </section>
      ...
      <section class="grid gap-3 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <UiInput v-model="model.officialWebsite" placeholder="请输入官网地址" />
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">预约说明</label>
          <UiInput v-model="model.reservationInfo" placeholder="请输入预约说明" />
        </div>
      </section>
      ...
      <section class="grid gap-3 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <input
            ...
            placeholder="请输入经度"
            @input="updateNumber('longitude', ($event.target as HTMLInputElement).value)">
        </div>
        <div class="space-y-2">
          ...
          <input
            ...
            class="h-9 w-full rounded-md border border-border bg-secondary px-3 text-sm text-foreground placeholder:text-muted-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
            placeholder="请输入纬度"
            @input="updateNumber('latitude', ($event.target as HTMLInputElement).value)">
        </div>
      </section>
      ...
      <section class="grid gap-3 md:grid-cols-3">
        <div class="space-y-2">
          ...
          <input
            ...
            placeholder="平方米"
            @input="updateNumber('landArea', ($event.target as HTMLInputElement).value)">
        </div>
        <div class="space-y-2">
          ...
          <input
            ...
            class="h-9 w-full rounded-md border border-border bg-secondary px-3 text-sm text-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
            placeholder="平方米"
            @input="updateNumber('buildingArea', ($event.target as HTMLInputElement).value)">
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">展览面积</label>
          <input
            :value="model.exhibitionArea ?? ''"
            type="number"
            step="0.01"
            class="h-9 w-full rounded-md border border-border bg-secondary px-3 text-sm text-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
            placeholder="平方米"
            @input="updateNumber('exhibitionArea', ($event.target as HTMLInputElement).value)">
        </div>
      </section>
      ...
      <section class="grid gap-3 md:grid-cols-3">
        <div class="space-y-2">
          ...
          <input
            :value="model.floorsAbove ?? ''"
            type="number"
            step="1"
            class="h-9 w-full rounded-md border border-border bg-secondary px-3 text-sm text-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
            placeholder="请输入地上层数"
            @input="updateNumber('floorsAbove', ($event.target as HTMLInputElement).value)">
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium text-foreground">地下层数</label>
          <input
            :value="model.floorsBelow ?? ''"
            type="number"
            step="1"
            class="h-9 w-full rounded-md border border-border bg-secondary px-3 text-sm text-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
            placeholder="请输入地下层数"
            @input="updateNumber('floorsBelow', ($event.target as HTMLInputElement).value)">
        </div>
        <div class="space-y-2">
      ...
      <section class="space-y-2">
        ...
        <textarea
          v-model="model.intro"
          rows="4"
          class="w-full rounded-xl border border-border bg-secondary px-3 py-2.5 text-sm leading-6 text-foreground transition-colors duration-200 focus:border-primary/45 focus:bg-accent"
          placeholder="请输入主体简介" />
      </section>
      ...
      <div class="flex flex-wrap justify-end gap-2 border-t border-border/70 pt-3">
        <UiButton variant="ghost" :disabled="props.submitting" @click="emit('reset')">
          重置
        </UiButton>
