...
- `packages/*` 当前只保留少量跨端契约与渲染预留，不提前抽离后台 UI 包。
...
| 包名 | 用途 |
| --- | --- |
| `@path-seeker/ts-shared` | 只保留已经稳定、且会被两个应用共同消费的类型与常量 |
| `@path-seeker/game-renderer` | 为后续题型预览 / 小程序共用渲染层保留最小协议入口 |
...
- `web-admin` 当前更偏信息架构与页面原型，后台组件先保留在 `apps/web-admin/app/components` 内部，不单独抽 `ui-admin` 包。
- `mp-wechat` 当前更偏脚手架，可从任务首页、章节地图、谜题渲染三条主线继续推进。
- 如果需要共享协议，优先沉淀到 `packages/ts-shared` 或 `packages/game-renderer`，前提是它们已经被两个应用共同消费。
- `docs/02-development-plan.md` 保留的是更长期的目标结构；当前仓库按“晚抽包、先在 app 内落地”的策略执行。
