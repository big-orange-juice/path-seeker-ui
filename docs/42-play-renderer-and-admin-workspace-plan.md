...
- [x] `SUPPORTED_INTERACTION_TYPES` / `resolveStageKind` 仅 4 型
- [x] 删除 2–9 契约、映射、renderer 文件与全仓引用
- [x] H5 / admin 去掉扩展题型分支与文案
- [x] H5：`1`/`6` brief **仅题面**；`10` 独享扫+播
- [x] typecheck / 构建通过；非法 type 不静默映射

**验收**：仓库内无 2–9 实现；1/6 无扫播 phase；10 有扫播；四型路由正确。

### Phase 2 · 后台工作台拆分（2–4d）

- [x] `RouteDetailDialog`：单击切换模拟器；编辑进 Dialog
- [x] 4 套节点 Form
- [x] 单节点手动保存 → 关 Dialog → 刷新 detail（Flow + 模拟器）
- [x] 模拟器 play only（无 studio 编辑）
- [x] 移除输入节流主保存路径
- [x] Agent 面板与刷新链路保持现状
...
- [x] admin 模拟器挂 `StagePlaySurface`（或等价），**按选中节点 kind 切换**
- [x] 选中 10：模拟器呈现扫+播（`FindScanPlayChain` 与 H5 共用）
- [x] 选中 1/6：模拟器仅题面，**不出现**扫/播；本地可试玩作答
- [x] 选中 11：仅解说
- [x] 禁提交/下一步（`canSubmit=false`，无预览提示条）
- [x] admin 全局 CSS 不污染题面（设备壳 scoped；题面样式在 game-renderer）
- [x] H5 与 B 端共用 `StagePlaySurface`（同源布局/黑金样式；不含星空背景）

**验收**：并排 H5 与 admin 模拟器，**同类型节点**观感一致（手测仍建议截图对照）。

### Phase 4 · 共享包去 studio + 收尾删除（1–2d）

- [x] 删除 studio 相关 API 与组件
- [x] 删除 `GameplayPreviewHost` 等后台编辑壳
- [x] 全仓确认无 2–9 / studio 残留
- [x] 更新 README 与相关 docs 题型表

**验收**：game-renderer 仅 4 play；H5 视觉无回归。

### Phase 5 · 数据对齐增强（可选）

- [x] 模拟器数据装配进一步对齐 H5（找一找增加 `video_url` 表单字段；config 多别名读取）
- [x] 10 的扫/播抽成可复用 `FindScanPlayChain` 供模拟器挂载（H5 页面壳样式仍冻结，可逐步切换）
- [ ] preview 只读会话（若后端提供）
...
- [x] 仓库仅存在 1/6/10/11 业务实现
- [x] 无 2–9 renderer / 契约 / 映射 / 产品文案（历史产品文档除外）
- [ ] 1/6/10/11 可走完整编辑 + 预览 + H5 主路径（**待浏览器回归**）

### 12.2 架构

- [x] 共享包无 studio 编辑面
- [x] 后台编辑仅 Form Dialog + 单节点手动保存
- [x] 保存后 Dialog 关闭且 Flow/模拟器随 detail 刷新
- [x] 模拟器与编辑器分离
- [x] Agent 行为与现网一致（前端未改策略）

### 12.3 视觉

- [ ] H5：1/6 题面样式不回归；10 扫播链路样式不回归；11 不回归（**待截图对照**）
- [ ] Admin 模拟器与 **同类型** H5 play 一致（扫组件同源；播片为共享链路，H5 壳未强制替换）
- [x] 1/6 模拟器无扫/播 UI；10 模拟器有扫/播
- [x] Admin 表单不出现在手机题面内

### 12.4 交互与组合

- [x] 模拟器可按当前节点试玩（1/6 可作答；10 可扫播；11 可听）
- [x] 模拟器无法服务端提交/下一步
- [x] 单节点保存后 detail 刷新生效
- [ ] 路线 `10→1→10→6` 在 H5 上连续体验为：扫播→答→扫播→拼图（**待手测**）
...
| 2026-07-22 | 完成四型硬删除、后台编辑/模拟拆分与静态构建验证；真实节点浏览器回归与 10 播片全链路同源抽取仍待完成。 |
| 2026-07-22 | 补齐模拟器可交互、`FindScanPlayChain` 扫播链路、非法 type 不静默映射、Flow 双击编辑、找一找 video_url 表单；Phase 3/5 主体完成，浏览器回归仍待手测。 |
