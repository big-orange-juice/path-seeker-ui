...
## 14. 2026-07-22 四型玩法收敛状态

本节以 `docs/42-play-renderer-and-admin-workspace-plan.md` 为准，覆盖本文早期 `1~9` 扩展题型描述。

| interactionType | 当前 H5 链路 | 当前后台模拟器 |
| --- | --- | --- |
| `1` 观察选择 | 进入 brief 后直接答题 | 仅答题题面，本地试玩 |
| `6` 纹样拼图 | 进入 brief 后直接拼图 | 仅拼图题面，本地试玩 |
| `10` 找一找 | 扫一扫 → 播片 → Submit 本站 | 扫一扫 → 本地播片，不提交、不进入下一节点 |
| `11` 解说导览 | 独立 narration 页 | 仅解说 play，不含编辑控件 |

`2/3/4/5/7/8/9` 已从共享契约、映射和 renderer 中移除，非法类型按 `null` 处理，不再降级成选择题。后台编辑已迁入节点 Form Dialog，保存后关闭并刷新路线详情。

已验证：H5 类型检查、后台生产构建、共享层历史 studio/renderer 残留扫描，以及真实游客路线的 `11` 解说分流。当前可用的游客样本未提供 `1/6/10` 节点，三者的真实数据端到端回归仍待具备对应路线后执行。