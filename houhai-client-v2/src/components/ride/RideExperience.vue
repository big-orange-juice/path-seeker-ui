...
import { ArrowLeft, ChevronDown, ChevronUp, Languages, MapPin, X } from 'lucide-vue-next'
...
const stopsCollapsed = shallowRef(false)
</script>
  ...
  <div v-else class="ride-experience">
    ...
    <main v-else-if="page === 'journey' && active && current" class="journey-page">
      ...
      <div class="journey-path">
        <button class="path-heading" :aria-expanded="!stopsCollapsed" :aria-label="translate(stopsCollapsed ? 'expandStops' : 'collapseStops')" @click="stopsCollapsed = !stopsCollapsed">
          <strong>{{ translate('stops') }}</strong>
          <span>{{ active.stops.length }}<ChevronDown v-if="stopsCollapsed" :size="15" /><ChevronUp v-else :size="15" /></span>
        </button>
        <StopTimeline v-show="!stopsCollapsed" :stops="active.stops" :current-id="current.id" :locale="locale" @select="ride.selectStop" />
