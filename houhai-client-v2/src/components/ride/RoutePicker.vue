...
const emit = defineEmits<{ select: [id: string]; start: []; resume: [] }>()
...
function pickCentered() {
  ...
  container.querySelectorAll<HTMLElement>('[data-route-id]').forEach(card => {
    ...
    if (distance < bestDistance) { bestDistance = distance; bestId = card.dataset.routeId ?? '' }
  })
  if (bestId && bestId !== props.selectedId) emit('select', bestId)
}
function onScroll() {
  clearTimeout(settle)
  settle = setTimeout(() => { if (Date.now() >= programmaticUntil) pickCentered() }, 170)
}
onMounted(() => revealSelected(true))
watch(() => props.selectedId, () => revealSelected())
watch(query, () => revealSelected(true))
...
<template>
  <div class="route-picker">
    ...
    <section class="route-deck" :aria-label="message(locale, 'routes')">
      ...
      <div ref="cards" class="route-cards" @scroll="onScroll">
        <button v-for="route in filtered" :key="route.id" :data-route-id="route.id" class="route-card" :class="{ selected: route.id === selectedId }" :aria-pressed="route.id === selectedId" @click="$emit('select', route.id)">
...
.route-picker{position:absolute;inset:0;display:flex;flex-direction:column;pointer-events:none}.route-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding:24px 32px;background:linear-gradient(#f8faf9f5,#f8faf9db,transparent)}.route-heading h1{font:600 28px var(--display);margin:0}.route-heading p{font-size:12px;color:#496459;margin:9px 0}.route-search{display:flex;align-items:center;gap:10px;padding:0 16px;min-height:48px;width:min(390px,45%);border:1px solid #d4e0d9;border-radius:14px;background:white;pointer-events:auto;color:var(--lake);box-shadow:0 4px 18px #183e4310}.route-search input{min-width:0;width:100%;border:0;background:none;outline-offset:6px;font-size:13px}.start-area{margin:auto;max-width:90%;text-align:center;pointer-events:auto}.start-ride{display:flex;justify-content:center;align-items:center;gap:15px;min-height:86px;max-width:100%;padding:20px 32px;background:var(--lake);color:white;border:5px solid #ffffffbd;border-radius:28px;box-shadow:0 12px 42px #183e4333;margin:0 auto}.start-ride strong{font-size:22px;line-height:1.3}.start-area p{max-width:400px;padding:7px 14px;border-radius:20px;background:#f8faf9ed;font-size:12px;line-height:1.5;color:var(--lake)}.return-active{border:1px solid #b8cbc0;padding:10px 18px;border-radius:20px;background:white;color:var(--lake);font-size:12px}.route-deck{padding:18px 0 30px;background:linear-gradient(transparent,#f8faf9e8 22%,#f8faf9);pointer-events:auto}.route-cards{display:flex;gap:14px;overflow-x:auto;scroll-snap-type:x mandatory;padding:4px calc((100% - min(72vw,330px)) / 2) 14px;scrollbar-width:none}.route-cards::-webkit-scrollbar{display:none}.route-card{flex:0 0 min(72vw,330px);display:flex;flex-direction:column;align-items:flex-start;gap:11px;min-height:204px;padding:18px 20px;text-align:left;background:#fffffff5;border:1px solid #ceddd3;border-radius:18px;color:var(--ink);scroll-snap-align:center;transform:scale(.94);opacity:.72;transition:transform .28s ease,opacity .28s ease,background .28s ease,border-color .28s ease}.route-card.selected{border:2px solid var(--lake);padding:17px 19px;background:#edf5ef;transform:scale(1);opacity:1}.route-guide{display:flex;align-items:center;gap:7px;width:100%;font-size:12px;color:var(--lake)}.route-guide .guide-check{margin-left:auto}.route-card strong{font:600 20px/1.35 var(--display)}.route-style{font-size:11px;color:#587466}.route-facts{display:flex;flex-wrap:wrap;align-items:center;gap:6px;font-size:11px}.route-stops{font-size:10px;line-height:1.7;color:#62786b;border-top:1px solid #d5e1d8;padding-top:9px;width:100%;margin-top:auto}.empty-result{background:white;padding:18px;border-radius:14px;font-size:13px}.route-dots{display:flex;justify-content:center;gap:6px;padding:10px 0 0}.route-dots span{width:6px;height:6px;border-radius:3px;background:#b9cec3;transition:width .25s ease,background .25s ease}.route-dots span.active{width:18px;background:var(--lake)}@media(max-width:760px){.route-heading{padding:18px 16px;display:block}.route-heading h1{font-size:24px}.route-heading p{font-size:11px}.route-search{width:100%;min-height:44px;margin-top:14px}.route-deck{padding:10px 0 24px}.route-card{min-height:215px;padding:16px}.route-card.selected{padding:15px}.route-card strong{font-size:19px}.start-ride{min-height:74px;padding:15px 24px;border-radius:24px}.start-ride strong{font-size:19px}.start-area p{font-size:11px}.route-cards{gap:12px}}@media(min-width:1000px){.route-cards{padding-left:calc((100% - min(50vw,440px)) / 2);padding-right:calc((100% - min(50vw,440px)) / 2)}.route-card{flex-basis:min(50vw,440px)}}
</style>
